 ...
 */
export async function VerifyTxFromContract(
    arweave: Arweave,
    contractId: string,
    verifyTx: string,
    dojBalance: number,
    ...
    let prev_tx: string, curr_tx: string, prev_tx_balance: number, curr_tx_balance: number, doj_prev_balance: number, doj_curr_balance: number
    if (!height) {
    ...
    for (let txInfo of txInfos) {
        ...
        state = result.state;
        if (currentTx.id !== verifyTx) {
            prev_tx_balance = state.balances[txOwner]
            doj_prev_balance = state.balances[dojAdrr]
        } else if (currentTx.id === verifyTx) {
            curr_tx_balance = state.balances[txOwner]
            doj_curr_balance = state.balances[dojAdrr]
            curr_tx = currentTx.id
        }
    ...
    console.log("curr_tx_bal", curr_tx_balance, "prev_tx_bal", prev_tx_balance, "doji_curr", doj_curr_balance, "doj balance", dojBalance, "curr_tx", curr_tx, "verify_tx", verifyTx, "transfer_amt", transferAmt);
    
    return (( prev_tx_balance - curr_tx_balance === transferAmt ) && ( doj_curr_balance - dojBalance === transferAmt ) && ( curr_tx === verifyTx ))
}
...
async function fetchTransactions(arweave: Arweave, contractId: string, height: number): Promise<GQLEdgeInterface[]> {
